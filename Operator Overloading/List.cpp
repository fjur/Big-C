...
void Iterator::next(){
    assert(position != NULL); //illegal to dereference null pointer
...
Iterator Iterator::operator--(int unused){
    ...
    //passing back a copy of iter and then stepping back in list
    
    Iterator temp = *this; // getting an object, not pointer since affects carry though
    assert(position != container->first);
    if (position == NULL) {
        position = container->last;
    } else{
